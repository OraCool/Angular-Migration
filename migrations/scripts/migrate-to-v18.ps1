<#
.SYNOPSIS
    Migrates Angular project to version 18.

.DESCRIPTION
    Performs complete Angular 18 migration including package updates,
...
.EXAMPLE
    .\migrate-to-v18.ps1 -ProjectPath "C:\MyProject" -CommitSteps
...
Write-Host "  Angular 18 Migration" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
# Confirm migration
Write-WarningMessage "This will migrate your project to Angular 18."
...
try {
    ...
    if ($result.Success) {
        Write-Host ""
        Write-Success "Migration to Angular 18 completed successfully!"
        Write-Host ""
        ...
        Write-InfoMessage "  3. Commit the changes: git add . && git commit -m 'chore: migrate to Angular 18'"
        Write-Host ""
        Write-InfoMessage "To continue migration:"
        Write-InfoMessage "  .\migrate-to-v19.ps1 -ProjectPath '$ProjectPath'"
        exit 0
    }
    else {
        Write-Host ""
        ...
        Write-InfoMessage "  2. Check migration documentation: migrations/docs/03-migrate-to-angular-18.md"
